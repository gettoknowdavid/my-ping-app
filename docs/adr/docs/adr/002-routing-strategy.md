...
- Use [`go_router`](https://pub.dev/packages/go_router) with [
  `go_router_builder`](https://pub.dev/packages/go_router_builder) for type-safe, code-generated routes.
...
+ Requires [`build_runner`](https://pub.dev/packages/build_runner) step when adding/changing routes